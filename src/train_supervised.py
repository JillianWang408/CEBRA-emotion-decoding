...
def main():
    ...
    # === Model ===
    neural_model = init_model(
        name="offset10-model",
        num_neurons=datasets.neural.shape[1],
        num_units=256,
    ...
    # Save model
    MODEL_WEIGHTS_PATH.parent.mkdir(exist_ok=True, parents=True)
    torch.save(solver.model.state_dict(), MODEL_WEIGHTS_PATH)
    print("Model training complete. Saved to:", MODEL_WEIGHTS_PATH)
